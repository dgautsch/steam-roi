...
class SteamAPI {
  ...
  constructor (key, cache = {}) {
    if (!key)
      console.log(
        'no key provided | some methods may not work | go get one > https://goo.gl/DfNy5s'
      )
    this.baseURL = 'https://api.steampowered.com'
    this.key = key
    this.headers = {
      'User-Agent': `SteamAPI/1 https://www.npmjs.com/package/steamapi`
    }
    this.options = {
  ...
  async resolve (info) {
    ...
    if (steamURL) {
      const json = await fetch(
        `${this.baseURL}/ISteamUser/ResolveVanityURL/v1?key=${this.key}&vanityurl=${steamURL}`,
        this.headers
      )
  ...
  async getAppList () {
    const json = await fetch(
      `${this.baseURL}/ISteamApps/GetAppList/v2`,
      this.headers
    )
  ...
  async getGameAchievements (app) {
    ...
    const json = await fetch(
      `${this.baseURL}/ISteamUserStats/GetGlobalAchievementPercentagesForApp/v2?gameid=${app}`,
      this.headers
    )
  ...
  async getGameDetails (app, force) {
    ...
    if (this.options.enabled) {
      if (
        force ||
        (this.cache.has(app) && this.cache.get(app).expires <= Date.now()) ||
        !this.cache.has(app)
      ) {
        const json = await fetch(
  ...
  async getGameNews (app) {
    if (isNaN(app)) throw new Error('no appid provided')
    const json = await fetch(
      `${this.baseURL}/ISteamNews/GetNewsForApp/v2?appid=${app}`,
      this.headers
    )
  ...
  async getGamePlayers (app) {
    if (isNaN(app)) throw new Error('no appid provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUserStats/GetNumberOfCurrentPlayers/v1?appid=${app}`,
      this.headers
    )
  ...
  async getGameSchema (app) {
    if (isNaN(app)) throw new Error('no appid provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUserStats/GetSchemaForGame/v2?key=${this.key}&appid=${app}`,
      this.headers
    )
  ...
  async getServers (host) {
    ...
    const json = await fetch(
      `${this.baseURL}/ISteamApps/GetServersAtAddress/v1?addr=${host}`,
      this.headers
    )
  ...
  async getUserAchievements (id, app) {
    ...
    if (isNaN(app)) throw new Error('no appid provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUserStats/GetPlayerAchievements/v1?key=${this.key}&steamid=${id}&appid=${app}`,
      this.headers
    )
  ...
  async getUserBadges (id) {
    ...
    const json = await fetch(
      `${this.baseURL}/IPlayerService/GetBadges/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
  ...
  async getUserBans (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUser/GetPlayerBans/v1?key=${this.key}&steamids=${id}`,
      this.headers
    )
  ...
  async getUserFriends (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUser/GetFriendList/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
  ...
  async getUserGroups (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUser/GetUserGroupList/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
  ...
  async getUserLevel (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/IPlayerService/GetSteamLevel/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
  ...
  async getUserOwnedGames (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/IPlayerService/GetOwnedGames/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
  ...
  async getUserRecentGames (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/IPlayerService/GetRecentlyPlayedGames/v1?key=${this.key}&steamid=${id}`,
      this.headers
    )
    return json.response.games.map(game => ({
      appID: game.appid,
  ...
  async getUserServers (hide = false, key = this.key) {
    const json = await fetch(
      `${this.baseURL}/IGameServersService/GetAccountList/v1?key=${key}`,
      this.headers
    )
  ...
  async getUserStats (id, app) {
    ...
    if (isNaN(app)) throw new Error('no appid provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUserStats/GetUserStatsForGame/v2?key=${this.key}&steamid=${id}&appid=${app}`,
      this.headers
    )
  ...
  async getUserSummary (id) {
    if (!idReg.test(id)) throw new Error('no id provided')
    const json = await fetch(
      `${this.baseURL}/ISteamUser/GetPlayerSummaries/v2?key=${this.key}&steamids=${id}`,
      this.headers
    )
